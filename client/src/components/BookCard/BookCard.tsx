...
const BookCard: React.FC<BookCardProps> = ({ id, title, author, city, photoUrl, forFree }) => {
  ...
  const toggleLike = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      if (isLiked) await removeFromWishlist(id);
  ...
  return (
    <div className="book-card" onClick={() => navigate(`/book/${id}`)}>
      <div className="book-card__image">
        ...
        {forFree && <span className="book-card__free-badge">Безкоштовно</span>}
      </div>
